...
def build_payload(phone_number: str) -> dict:
    return {
        ...
        "metadata": {"experiment": "recipient_result_schema_validation"},
    }
...
def main():
    ...
    print("\n" + "=" * 60)
    print("RECIPIENT_RESULT_SCHEMA VALIDATION — single real call")
    print("=" * 60)
    print("Request body about to be sent:")
    print(json.dumps(payload, indent=2))
    print(
        "\nNote: this uses the SAME 'recipients' (plural) shape already "
        "proven to work in production. The only intentional difference "
        "from that known-working request is the presence of "
        "'recipient_result_schema' and a trivial placeholder in "
        "'result_schema'. region/locale remain intentionally absent, same "
        "as every prior experiment."
    )
    ...
    idempotency_key = f"careflow_validation_recipient_result_schema_{int(time.time())}"
    creation_result = place_call(payload, idempotency_key)

    creation_output_path = SCRIPT_DIR / "recipient_result_schema_creation_response.json"
    ...
    final_output_path = SCRIPT_DIR / "recipient_result_schema_final_result.json"
